...
├── zerx-spring/                            # Spring 层（开发中）
│   ├── zerx-spring-web/                    #   Web 层增强（统一响应、全局异常、请求上下文等）
│   ├── zerx-spring-data/                   #   数据访问增强（DynamicQuery、归档、数据权限等）
│   ├── zerx-spring-cache/                  #   多级缓存（Caffeine + Redis）
│   └── zerx-spring-security/               #   认证授权（JWT + Spring Security）
├── docs/                                   # 项目文档
