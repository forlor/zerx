 ...
 * <h3>URL 构建：</h3>
 * <p>
 * 未配置自定义域名时，使用虚拟托管式（virtual-hosted-style）URL：
 * {@code https://{bucket}.{endpoint}/{objectKey}}
 * </p>
 *
 * <h3>用户元数据：</h3>
 * <p>
...
public class AliyunOssStorageService extends AbstractOssStorageService {
     ...
     * 构造阿里云 OSS 存储服务。
     * <p>
     * 使用 {@link OSSClientBuilder} 创建 OSS 客户端实例。客户端在构造后立即可用，
     * 无需额外初始化。调用方需自行管理客户端的生命周期（如通过 Spring {@code @PreDestroy} 关闭）。
     * </p>
     *
     * @param properties 阿里云 OSS 配置属性，必须包含 {@code endpoint}、{@code accessKey}、{@code secretKey}、{@code bucket}
     ...
     * {@inheritDoc}
     * <p>
     * 使用 {@code ossClient.putObject(bucket, key, inputStream, metadata)} 上传对象。
     * 自定义元数据条目中 {@code x-amz-meta-} 前缀会自动转换为阿里云的 {@code x-oss-meta-} 前缀。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 通过 {@code ossClient.getObjectMetadata(bucket, key)} 获取对象元数据，
     * 并从用户自定义元数据中提取原始文件名。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 通过 {@code ossClient.getObject(bucket, key)} 获取对象，封装为 {@link OssObject}。
     ...
     * {@inheritDoc}
     * <p>
     * 通过 {@code ossClient.doesObjectExist(bucket, key)} 判断对象是否存在。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 通过 {@code ossClient.deleteObject(bucket, key)} 删除单个对象。
     * 删除不存在的对象不会抛出异常（幂等操作）。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 通过 {@code ossClient.deleteObjects(new DeleteObjectsRequest(bucket).withKeys(keys))}
     * 批量删除对象。返回实际成功删除的对象数量。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 通过 {@code ossClient.copyObject(new CopyObjectRequest(sourceBucket, sourceKey, bucket, targetKey))}
     * 复制对象，然后对目标对象执行 stat 以获取最新元数据。
     * 支持跨桶复制（如从暂存桶到主桶）。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 使用 {@code ossClient.generatePresignedUrl(bucket, key, expiryDate, HttpMethod.PUT)}
     * 生成预签名上传 URL。自定义元数据头的 {@code x-amz-meta-} 前缀会自动转换为
     * 阿里云的 {@code x-oss-meta-} 前缀以保证签名校验通过。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 使用 {@code ossClient.generatePresignedUrl(bucket, key, expiryDate, HttpMethod.GET)}
     * 生成预签名下载 URL。
     * </p>
     */
    @Override
     ...
     * 否则使用阿里云虚拟托管式（virtual-hosted-style）URL：
     * {@code https://{bucket}.{endpoint}/{objectKey}}。
     * 自动去除 endpoint 中可能包含的协议前缀。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 通过 {@code ossClient.getObjectMetadata(bucket, key).getUserMetaData()} 获取
     * 阿里云用户自定义元数据。返回的 Map 中键名自动添加 {@code x-amz-meta-} 前缀，
     * 以保持与模板层的一致性。
     * </p>
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * 使用 {@code ossClient.listObjectsV2} 列出暂存前缀下的所有对象，
     ...
     * 去除 ETag 值两端的引号。
     * <p>
     * 阿里云 OSS 返回的 ETag 可能包含双引号（如 {@code "abc123def456"}），
     * 需要去除以便统一存储和使用。
     * </p>
     *
    ...
    /**
     * 去除字符串中的协议前缀（http:// 或 https://）。
     *
     * @param endpoint 原始端点字符串
     * @return 去除协议前缀后的字符串
     */
